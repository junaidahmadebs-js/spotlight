...
const LiquidityChart: React.FC<LiquidityChartProps> = ({ settings }) => {
  ...
  const generatePath = () => {
    ...
    if (settings.type === "Line") {
      return data
        .map(
          (p, i) => `${i === 0 ? "M" : "L"} ${getXPos(i)} ${getYPos(p.value)}`
        )
        .join(" ");
    }
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <div style={{ position: "absolute", top: getYPos(800) - 8, left: 0 }}>
            800 Dirham (000)
          </div>
          {Y_GRID_LINES.slice(1).map((val) => (
            <div
              key={val}
              style={{ position: "absolute", top: getYPos(val) - 8, left: 0 }}
            >
              {val}
            </div>
          ))}
        ...
        >
          {months.map((month) => (
            <span key={month} style={{ margin: "0 1px" }}>
              {month}
            </span>
          ))}
        </div>
